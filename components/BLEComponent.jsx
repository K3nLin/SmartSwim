...
const BleComponent = ({ setConnectionStatus, sendStartSignalRef, sendStopSignalRef, readDataRef, stopReadDataRef }) => {
  ...
  const sendSignal = async(startStopUUID, encodedMessage) => {
    try {
      ...
      startStopUUID === START_SIGNAL_UUID ? console.log("Start signal sent!") : console.log("Stop signal sent!");
      Alert.alert("Success", `${startStopUUID === START_SIGNAL_UUID ? 'Start' : 'Stop'} signal sent to ESP32.`);
    } catch (error) {
      throw error;
    }
  }
  ...
  const readData = async () => {
    ...
    try {
      ...
      const newSubscription = device.monitorCharacteristicForService(serviceUUID, characteristicUUID, (error, characteristic) => {
        if (error) {
